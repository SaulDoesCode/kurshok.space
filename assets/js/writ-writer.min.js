...
</article>`.collect();writingPad.value="",app.titleInput=titleInput,app.writingPad=writingPad,app.tagInput=tagInput,app.doob=async(t=10)=>{let e=0;for(;e<t;)++e,app.clearEditor(),app.titleInput.value="Doob # "+e,app.writingPad.value="Doob # "+e,app.tagInput.value="test",await app.editorPushWrit()},(app.wwTS=app.setupToggleSituation(wwLauncher,wwView,"body",{viewOutAnimation:"fade-out 220ms ease-out",delayRemoveMS:220,background:!0})).toggleView();const writListEntry=(t,e,a=!1)=>div({class:{"wl-entry":!0,unpushed:null==e},$:a?void 0:writList,attr:{wid:null==e?t:e}},i=>(a&&writList.prepend(i),[i.titleSpan=span(t),div(()=>{const a=span({class:"delete-writ",attr:{title:"Double click/tap to delete writ"}},"🗑");let r,n=0;const l=d.on.pointerup(a,async s=>{if(clearTimeout(r),2!=++n)return 1==n&&a.classList.add("prep"),void(r=setTimeout(()=>{a.classList.remove("prep"),n=0},900));n=0;try{if(l.off(),null!=e){(await app.deleteWritRequest(e)).ok&&(df.remove(i),app.ww.active&&app.ww.active.id==e&&app.clearEditor(),delete app.ww.writs[e],app.emit("postEdit:"+e,!1))}else await localforage.removeItem("unpushed:"+t),delete app.ww.unpushed[t],df.remove(i),app.ww.active&&app.ww.active.title==t&&app.clearEditor()}catch(s){l.on(),console.error("Well, that didn't work: "+s)}});return a})]));app.ww={writs:{},unpushed:{}},app.pushWrit=async(t,e,a,i={})=>{if(1===a.length&&""===a[0])throw new Error("posts need at least one tag");const r={title:t,raw_content:e.trim(),tags:a,kind:"post",public:!0,viewable_by:[],...i},n=await app.jsonPut("/writ",r),l=await n.json();if(l.ok){if(null!=r.id){const t=writList.querySelector(`[wid="${r.id}"]`);t&&app.editableWritQuery({author_name:app.user.username,with_raw_content:!1,ids:[r.id]}).then(async e=>{if(!d.isArr(e))return app.toast.error("Failed to fetch your editable writs"),console.error("failed to fetch user's editable writs");for(const a of e)a.raw_content=r.raw_content,app.ww.writs[a.id]=a,t.titleSpan.textContent=a.title,app.emit("postEdit:"+a.id,a)})}else r.id=l.data.id,r.posted=l.data.posted,r.slug=l.data.slug,app.emit.newPost(r.id),app.ww.writs[r.id]=r,writListEntry(r.title,r.id),app.clearEditor();return Promise.resolve(l.data)}return Promise.reject(l)},app.deleteWritRequest=t=>app.txtDelete("/writ",t),app.editableWritQuery({author_name:app.user.username,with_raw_content:!1}).then(async t=>{if(!d.isArr(t))return app.toast.error("Failed to fetch your editable writs"),console.error("failed to fetch user's editable writs");for(const e of t)app.ww.writs[e.id]=e,writListEntry(e.title,e.id);for(const t of await(await app.localForage()).keys())if(t.includes("unpushed:")){const e=JSON.parse(await localforage.getItem(t));app.ww.unpushed[e.title]=e,writListEntry(e.title)}}),app.rawContentRequest=async t=>{const e=await fetch("/writ-raw-content/"+t);return await e.json()},app.localForage=()=>app.localForageLoaded?Promise.resolve(window.localforage):new Promise(t=>app.once.localForageLoaded(()=>t(window.localforage))),d.run(async()=>{await app.loadScriptsThenRunSequentially(!0,"/js/localforage.min.js"),app.emit("localForageLoaded",app.localForageLoaded=!0)}),d.on.pointerup(writList,t=>{if(t.target.classList.contains("selected")||t.target.parentElement.classList.contains("selected"))return;let e=t.target.getAttribute("wid")||t.target.parentElement.getAttribute("wid");if(null!=e){const t=app.ww.active=app.ww.writs[e]||app.ww.unpushed[e];null==app.ww.writs[e]?pushWritBtn.after(saveLocallyBtn):df.remove(saveLocallyBtn),app.ww.selectedWLE&&app.ww.selectedWLE.classList.remove("selected"),app.ww.selectedWLE=d.query(`[wid="${e}"]`),app.ww.selectedWLE.classList.add("selected");let a=null==t.raw_content;null==t.raw_content&&app.rawContentRequest(t.id).then(e=>{e.ok||(writingPad.value="Ok, so loading failed for some reason, you should reload or something, it's probably a client side error, or less likely, a database issue - "+(e.status||"")),writingPad.value=t.raw_content=e.data,a=!1}),titleInput.value=t.title,writingPad.value=t.raw_content||"Hang tight, the content is loading...",setTimeout(()=>{if(a){let e=0;const i="Hang tight, the content is loading";let r=setInterval(()=>{if(a){let t="";d.each(e++,()=>t+="."),writingPad.value=i+t,4==e&&(e=0)}else clearInterval(r),e=0,writingPad.value=t.raw_content},220)}},220),tagInput.value=t.tags.join(", "),isPublicCheckbox.checked=t.public,isCommentableCheckbox.checked=t.commentable,pushWritBtn.textContent="Update"}}),app.clearEditor=()=>{app.ww.active&&(app.ww.active=null),titleInput.value=writingPad.value=tagInput.value="",isPublicCheckbox.checked=isCommentableCheckbox.checked=!0,app.ww.selectedWLE&&(app.ww.selectedWLE.classList.remove("selected"),app.ww.selectedWLE=null,pushWritBtn.textContent="Push"),pushWritBtn.after(saveLocallyBtn)},app.gatherWritFromWriter=()=>{const t=titleInput.value.trim(),e=writingPad.value.trim(),a=isPublicCheckbox.checked,i=isCommentableCheckbox.checked;return{title:t,raw_content:e,tags:tagInput.value.split(",").map(t=>t.trim()),ops:{is_md:!0,public:a,commentable:i}}},app.editorPushWrit=async()=>{let t;console.log("trying to push writ...");const e=app.gatherWritFromWriter();return app.ww.active&&(e.ops.id=app.ww.active.id),t=await app.pushWrit(e.title,e.raw_content,e.tags,e.ops),t&&null!=t.title&&app.toast.msg("success, writ posted: "+t.title),t},d.on.pointerup(saveLocallyBtn,async t=>{if(app.ww.active&&null!=app.ww.active.id)return;const{title:e,raw_content:a,tags:i,ops:r}=app.gatherWritFromWriter(),n={title:e,raw_content:a,tags:i,...r};await localforage.setItem("unpushed:"+e,JSON.stringify(n)),app.ww.active=app.ww.unpushed[e]=n,app.ww.selectedWLE&&app.ww.selectedWLE.classList.remove("selected"),app.ww.selectedWLE=writListEntry(e),app.ww.selectedWLE.classList.add("selected")}),d.on.pointerup(clearEditorBtn,app.clearEditor),d.on.pointerup(pushWritBtn,t=>{app.editorPushWrit()}),d.on.keydown(tagInput,t=>{"Enter"===t.key&&(app.editorPushWrit(),t.preventDefault(),titleInput.focus())}),d.on.input(titleInput,t=>{"Enter"===t.key?(writingPad.focus(),t.preventDefault()):app.ww.active&&app.ww.selectedWLE&&(app.ww.selectedWLE.titleSpan.textContent=titleInput.value)});
