...
</article>`.collect();writingPad.value="",(app.wwTS=app.setupToggleSituation(wwLauncher,wwView,"body",{viewOutAnimation:"fade-out 220ms ease-out",delayRemoveMS:220})).toggleView();const writListEntry=(t,e)=>div({class:"wl-entry",$:writList,attr:{wid:e}},span(t));app.user.writs={},app.ww={},app.pushWrit=async(t,e,i,a={})=>{const s={title:t,raw_content:e.trim(),tags:i,kind:"post",public:!0,viewable_by:[]};Object.assign(s,a);const l=await app.jsonPut("/writ",s),c=await l.json();return c.ok?Promise.resolve(c.data):Promise.reject(c)},app.editableWritQuery({author_name:app.user.username,with_raw_content:!0}).then(async t=>{console.log(t),d.isArr(t)||console.error("failed to fetch user's editable writs"),t.forEach(t=>{var e,i;app.user.writs[t.id]=t,e=t.title,i=t.id,div({class:"wl-entry",$:writList,attr:{wid:i}},span(e))})}),d.on.click(writList,t=>{if(t.target.classList.contains("selected")||t.target.parentElement.classList.contains("selected"))return;let e=t.target.getAttribute("wid")||t.target.parentElement.getAttribute("wid");if(null!=e){const t=app.ww.active=app.user.writs[e];app.ww.selectedWLE&&app.ww.selectedWLE.classList.remove("selected"),app.ww.selectedWLE=d.query(`[wid="${e}"]`),app.ww.selectedWLE.classList.add("selected"),titleInput.value=t.title,writingPad.value=t.raw_content,tagInput.value=t.tags.join(", "),isPublicCheckbox.checked=t.commentable,isCommentableCheckbox.checked=t.public,console.log("got one:",t)}}),d.on.click(clearEditorBtn,t=>{app.ww.active&&(app.ww.active=null),titleInput.value=writingPad.value=tagInput.value="",isPublicCheckbox.checked=isCommentableCheckbox.checked=!0,app.ww.selectedWLE&&(app.ww.selectedWLE.classList.remove("selected"),app.ww.selectedWLE=null)}),d.on.click(pushWritBtn,async t=>{try{let t;console.log("trying to push writ...");const e=titleInput.value.trim(),i=writingPad.value.trim(),a=isPublicCheckbox.checked,s=isCommentableCheckbox.checked,l=tagInput.value.split(",").map(t=>t.trim()),c={is_md:!0,public:a,commentable:s};app.ww.active&&(c.id=app.ww.active.id),t=await app.pushWrit(e,i,l,c),null!=t&&console.log(t)}catch(t){console.error(t)}}),d.on.click(deleteWritBtn,t=>{});