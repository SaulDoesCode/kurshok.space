...
</article>`.collect();writingPad.value="",(app.wwTS=app.setupToggleSituation(wwLauncher,wwView,"body",{viewOutAnimation:"fade-out 220ms ease-out",delayRemoveMS:220})).toggleView();const writListEntry=(t,e)=>div({class:"wl-entry",$:writList,attr:{wid:e}},span(t),div(()=>{const t=span({class:"delete-writ",attr:{title:"Double click/tap to delete writ"}},"🗑");let i,a=0;const r=d.on.pointerup(t,async s=>{if(clearTimeout(i),2!=++a)return 1==a&&t.classList.add("prep"),void(i=setTimeout(()=>{t.classList.remove("prep"),a=0},900));a=0;try{r.off();(await app.deleteWritRequest(e)).ok&&(df.remove(d.query(`[wid="${e}"]`)),app.ww.active&&app.ww.active.id==e&&app.clearEditor(),delete app.ww.writs[e])}catch(s){r.on(),console.error("Well, that didn't work: "+s)}});return t}));app.ww={writs:{}},app.pushWrit=async(t,e,i,a={})=>{const r={title:t,raw_content:e.trim(),tags:i,kind:"post",public:!0,viewable_by:[],...a},s=await app.jsonPut("/writ",r),n=await s.json();return n.ok?Promise.resolve(n.data):Promise.reject(n)},app.deleteWritRequest=t=>app.txtDelete("/writ",t),app.editableWritQuery({author_name:app.user.username,with_raw_content:!1}).then(async t=>{d.isArr(t)||console.error("failed to fetch user's editable writs"),console.log(t);for(const e of t)app.ww.writs[e.id]=e,writListEntry(e.title,e.id)}),app.rawContentRequest=async t=>{const e=await fetch("/writ-raw-content/"+t);return await e.json()},d.on.pointerup(writList,t=>{if(t.target.classList.contains("selected")||t.target.parentElement.classList.contains("selected"))return;let e=t.target.getAttribute("wid")||t.target.parentElement.getAttribute("wid");if(null!=e){const t=app.ww.active=app.ww.writs[e];app.ww.selectedWLE&&app.ww.selectedWLE.classList.remove("selected"),app.ww.selectedWLE=d.query(`[wid="${e}"]`),app.ww.selectedWLE.classList.add("selected");let i=null==t.raw_content;null==t.raw_content&&app.rawContentRequest(t.id).then(e=>{e.ok||(writingPad.value="Ok, so loading failed for some reason, you should reload or something, it's probably a client side error, or less likely, a database issue - "+(e.status||"")),writingPad.value=t.raw_content=e.data,i=!1}),titleInput.value=t.title,writingPad.value=t.raw_content||"Hang tight, the content is loading...",setTimeout(()=>{if(i){let e=0;const a="Hang tight, the content is loading";let r=setInterval(()=>{if(i){let t="";d.each(e++,()=>t+="."),writingPad.value=a+t,4==e&&(e=0)}else clearInterval(r),e=0,writingPad.value=t.raw_content},220)}},220),tagInput.value=t.tags.join(", "),isPublicCheckbox.checked=t.commentable,isCommentableCheckbox.checked=t.public,pushWritBtn.textContent="Update"}}),app.clearEditor=()=>{app.ww.active&&(app.ww.active=null),titleInput.value=writingPad.value=tagInput.value="",isPublicCheckbox.checked=isCommentableCheckbox.checked=!0,app.ww.selectedWLE&&(app.ww.selectedWLE.classList.remove("selected"),app.ww.selectedWLE=null,pushWritBtn.textContent="Push")},app.editorPushWrit=async()=>{let t;console.log("trying to push writ...");const e=titleInput.value.trim(),i=writingPad.value.trim(),a=isPublicCheckbox.checked,r=isCommentableCheckbox.checked,s=tagInput.value.split(",").map(t=>t.trim()),n={is_md:!0,public:a,commentable:r};if(app.ww.active&&(n.id=app.ww.active.id),t=await app.pushWrit(e,i,s,n),null!=t&&t.ok)return console.log(t),t},d.on.pointerup(clearEditorBtn,app.clearEditor),d.on.pointerup(pushWritBtn,t=>{app.editorPushWrit()});
