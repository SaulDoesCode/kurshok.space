...
const domfn = {
  ...
  class (node, c, state) {
    if (node && c != null && node.classList) {
        ...
        if (d.isArr(c)) {
            const method = typeof state !== 'boolean' ? 'toggle' : state ? 'add' : 'remove'
  ...
  emit: (node, type, detail) => (node.dispatchEvent(typeof type !== 'string' ? type : new CustomEvent(type, {detail})), node),
  append: (node, ...children) => (d.attach(node, 'appendChild', ...children), node),
  prepend: (node, ...children) => (d.attach(node, 'prepend', ...children), node),
  appendTo: (node, host) => (d.attach(node, 'appendChild', host), node),
  prependTo: (node, host) => (d.attach(node, 'prepend', host), node),
  clear: node => (node[d.isInput(node) ? 'value' : 'textContent'] = '', node),
  ...
  remove (node, after) {
    ...
    else if (d.isNum(after)) setTimeout(() => domfn.remove(node), after)
    ...
    function d(tag, ops, ...children) {
        ...
        if (opsisObj) {
            ...
            for (const key in ops) {
                ...
                } else if (key in d.domfn) {
                    val = d.isArr(val) ? d.domfn[key](el, ...val) : d.domfn[key](el, val)
