...
pub async fn render_post(
    ...
    render_template(&mut ctx, "post.html",
...
pub async fn render_post_by_slug(
    ...
    let writ_id = if let Ok(Some(writ_id)) = orc.slugs.get(slug_key.as_bytes()) {
        ...
        return render_404(&mut ctx, "That's a bad slug, we couldn't find any posts matching it.", orc.dev_mode);
    };
...
fn render_template(
    ...
    match TEMPLATES.read().render(name, &ctx) {
        ...
        Err(err) => if dev_mode {
            ...
            HttpResponse::InternalServerError()
                .content_type("text/plain")
                .body(&format!("The bloody {} template is broken or missing! :( We have failed you.", name))
        }
    }
}
